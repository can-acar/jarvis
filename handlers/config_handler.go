...
func HandleSetConfig(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("Failed to set configuration: %v", err)), nil
	}
...
func HandleAddAllowedDirectory(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	err = common.AddAllowedDirectory(directory)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("Failed to add allowed directory: %v", err)), nil
	}
